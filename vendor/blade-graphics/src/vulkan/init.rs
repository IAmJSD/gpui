...
/// The device extensions blade needs, each with the core Vulkan version
/// that absorbed it. A driver at or past that version need not advertise
/// the extension (Android's emulator does not), so it is only required,
/// and only enabled, below that version.
const REQUIRED_DEVICE_EXTENSIONS: &[(&ffi::CStr, u32)] = &[
    (vk::EXT_INLINE_UNIFORM_BLOCK_NAME, vk::API_VERSION_1_3),
    (vk::KHR_TIMELINE_SEMAPHORE_NAME, vk::API_VERSION_1_2),
    (vk::KHR_DESCRIPTOR_UPDATE_TEMPLATE_NAME, vk::API_VERSION_1_1),
    (vk::KHR_DYNAMIC_RENDERING_NAME, vk::API_VERSION_1_3),
];

/// Loads a device function by its extension name, falling back to the
/// core name (the same without the `KHR` suffix) on drivers that have the
/// feature as core Vulkan but do not expose the extension entry points.
unsafe fn load_device_fn(
    instance: &ash::Instance,
    device: vk::Device,
    name: &ffi::CStr,
) -> *const ffi::c_void {
    let load = |name: &ffi::CStr| -> *const ffi::c_void {
        match instance.get_device_proc_addr(device, name.as_ptr()) {
            Some(function) => function as *const ffi::c_void,
            None => std::ptr::null(),
        }
    };
    ...
    if let Some(core_name) = name.to_bytes().strip_suffix(b"KHR") {
        if let Ok(core_name) = ffi::CString::new(core_name) {
            return load(&core_name);
        }
    }
    std::ptr::null()
}
...
unsafe fn inspect_adapter(
    ...
    let supported_extensions = supported_extension_properties
        ...
        .collect::<Vec<_>>();
    for &(extension, core_version) in REQUIRED_DEVICE_EXTENSIONS {
        if !supported_extensions.contains(&extension) && api_version < core_version {
            log::warn!(
...
impl super::Context {
    pub unsafe fn init(desc: crate::ContextDesc) -> Result<Self, NotSupportedError> {
        ...
        let device_core = {
            ...
            // What the driver advertises; a promoted extension it does
            // not is core, and enabling it would fail device creation.
            let supported_extension_properties = instance
                .core
            ...
            let mut device_extensions = REQUIRED_DEVICE_EXTENSIONS
                .iter()
                .filter(|(extension, _)| supported_extensions.contains(extension))
        ...
        let device = super::Device {
            ...
            timeline_semaphore: khr::timeline_semaphore::DeviceFn::load(|name| {
                load_device_fn(&instance.core, device_core.handle(), name)
            }),
            dynamic_rendering: khr::dynamic_rendering::DeviceFn::load(|name| {
                load_device_fn(&instance.core, device_core.handle(), name)
            }),
